...
export default function DataFlowBackground() {
  ...
  const noise = (x: number, y: number) => {
    // 简化的噪声函数
    return Math.sin(x * 0.01) * Math.cos(y * 0.01) * Math.sin((x + y) * 0.005) * 2
  }
  ...
  const distortPoint = (
    ...
    // 基础螺旋旋转
    const angle = rotation + Math.atan2(point.baseY - centerY, point.baseX - centerX)
    const distance = Math.sqrt(
      Math.pow(point.baseX - centerX, 2) + Math.pow(point.baseY - centerY, 2)
    )
  ...
  const drawGrid = (
    ...
    const distortedPoints = grid.points.map((point: any) => {
      const distorted = distortPoint(point, time, grid.centerX, grid.centerY, grid.rotation)
        ...
        if (x < cols - 1) {
          ...
          if (distortedPoints[nextColIndex]) {
            ...
            gradient.addColorStop(
              0,
              `hsla(${hue1}, 100%, 70%, ${0.3 + currentPoint.glitchIntensity})`
            )
            ...
            ctx.lineWidth =
              1 + Math.max(currentPoint.glitchIntensity, nextPoint.glitchIntensity) * 4
            ...
            if (Math.random() < 0.1 || glitchTimeRef.current > 0) {
              const glitchAmount =
                2 + Math.max(currentPoint.glitchIntensity, nextPoint.glitchIntensity) * 10
  ...
  const drawRaindrops = (ctx: CanvasRenderingContext2D, time: number, height: number) => {
    raindropRef.current.forEach((drop, index) => {
      ...
      // 绘制拖尾
      ctx.beginPath()
      const tailGradient = ctx.createLinearGradient(drop.x, drop.y, drop.x, drop.y - drop.size * 5)
